...
from src.utils.exceptions import CancleRequestAndColldownError, MasterRequestAlreadyInProgressError, MasterRequestCooldownError, MasterRequestUniqueError,RoleNotAllowedError,ApplicationNoFound,ApplicationApproved
...
async def application(db : DbDep, user : UserDep,data : MasterCreateRequestSchema = Body(openapi_examples={"1" : {
    ...
    try:
        result = await MastersService(db).request_application(id_user=user.user_id,data=data,role = user.role)
        await db.commit()
        return {"data" : result}
        ...
        raise HTTPException(status_code=403, detail="Только пользователь может создать заявку.")
...
async def update_master(db : DbDep, master : MasterDep, data : MasterUpdateSchema = Body(openapi_examples={"1" : {
